...
async def upload_file(file: UploadFile = File(...)):
    ...
    return JSONResponse(content={
        ...
        "probabilities": [probability for _, probability in top_3_predictions],
    })

class SymptomResponse(BaseModel):
    answers: Dict[str, str]  # symptom name -> '1' or '0'
    diseases: List[str]  # candidate diseases returned by /upload
    probabilities: Optional[List[float]] = None  # the photo's confidence in each, returned by /upload

@router.post("/confirm_symptoms")
async def confirm_symptoms(data: SymptomResponse):
    print("Received Data:", data)

    confirmed_disease, severity = process_user_responses(data.diseases, data.answers, data.probabilities)
